...
def get_processed_file_path(message_date):
    return f'processed/{message_date.year}-{message_date.month}.json'

def load_existing_messages_from_s3(message_date):
    bucket = s3.Bucket(bucket_name)

    expected_file_path = get_processed_file_path(message_date)
...
def save_processed_messages_to_s3(messages, message_date):
    s3_path = get_processed_file_path(message_date)
